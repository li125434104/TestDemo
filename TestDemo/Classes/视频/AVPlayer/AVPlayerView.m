    ...
    _queryCacheOperation = [[WebCacheHelpler sharedWebCache] queryURLFromDiskMemory:_cacheFileKey cacheQueryCompletedBlock:^(id data, BOOL hasCache) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(!hasCache) {
                ...
                wself.sourceURL = [self urlScheme:@"streaming" withStr:wself.sourceURL.absoluteString];
            }else {
...
-(void)retry {
    [self cancelLoading];
    _sourceURL = [self urlScheme:_sourceScheme withStr:_sourceURL.absoluteString];
...
- (NSURL *)urlScheme:(NSString *)scheme withStr:(NSString *)str {
